...
resource "yandex_compute_instance" "platform" {
  name        = local.name_vm1 
  platform_id = var.vm_web_patform_id
  resources {
    cores         = var.vms_resources.vm_web_resources.cores
    memory        = var.vms_resources.vm_web_resources.mem
...
resource "yandex_compute_instance" "platform_2" {
  name        = local.name_vm2 
  platform_id = var.vm_db_patform_id
  resources {
    cores         = var.vms_resources.vm_db_resources.cores
    memory        = var.vms_resources.vm_db_resources.mem
